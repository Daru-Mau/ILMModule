...
// In obstacle_avoidance.ino we need to refer to the enum defined in the main file
// First, forward declare the enum type
enum AvoidanceState;

// Reference to the variable from integrated_movement.ino, using the proper type
extern AvoidanceState avoidanceState;

// The enum values are defined in integrated_movement.ino
// We'll use the actual enum values directly instead of constants
...
// Standard Arduino functions are already available through Arduino.h
// We only need to declare any non-standard functions
