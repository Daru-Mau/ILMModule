...
void print_ticket(int num, bool luck){
  ...
  if (!luck) {
    ...
    printer.feed(3);
    printer.write(0x1B); printer.write(0x6D); //Partial cut command
  } else {
    printer.println("You got the lucky ball! you will be called as ASAP!");
    printer.feed(1);
...
enum MasterState { S_BOOTING, S_IDLE, S_PRINT, S_EXTEND, S_WAIT_BLACK, S_RETRACT };
MasterState mState = S_BOOTING;
...
void loop() {
  ...
  switch (mState) {
    ...
    case S_IDLE:
      // TODO: if it is lunch time, send the START_MOVEMENT_CMD to localization
      break;

    case S_PRINT:
      //Printing ticket code
      // TODO: if lucky, change pritnting sequence
      print_ticket(currentPrintNum, 1);
